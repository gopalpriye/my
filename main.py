...
async def start(bot, message):
  ...
  keyboard = [
    [
      InlineKeyboardButton(" ♾️ Physics Wallah without Purchase ", callback_data="pwwp")
    ],
    [
      InlineKeyboardButton(" 🔖 Classplus without Purchase ", callback_data="cpwp")
    ],
    [
      InlineKeyboardButton("🎯 Appx Without Purchase sala sb iske liye pagal hai 😁", callback_data="appxwp")
    ]
  ]
                ...
                if total > 50:
                    ...
                    for file in files:
                        ...
                        try:
                            with open(file, 'rb') as f:
                                await editable.delete(True)
                                doc = await m.reply_document(document=f, caption=caption, thumb=bot_logo, file_name=f"paid course details.{file_ext}")
